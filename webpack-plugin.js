...
module.exports = class JSONManifestPlugin {
    ...
    apply(compiler) {
        compiler.hooks.compilation.tap('JSONManifestPlugin', compilation => {
            compilation.hooks.processAssets.tapPromise({name: 'JSONManifestPlugin', stage: webpack.Compilation.PROCESS_ASSETS_STAGE_REPORT }, async () => {
                const result = {manifestVersion: 1, version: this.version, entries: []};


                for(const entrypoint of compilation.entrypoints.values()) {
                    for(const chunk of entrypoint.chunks) {
                        result.entries = result.entries.concat([...chunk.files.values()].filter(val => val.endsWith(".js")))
                    }
                }
