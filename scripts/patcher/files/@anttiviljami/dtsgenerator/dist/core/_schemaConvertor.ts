...
export default class SchemaConvertor {
    ...
    constructor(
        private processor: WriteProcessor,
        private convertor: TypeNameConvertor = DefaultTypeNameConvertor,
        namespaceName?: string
    ) {
        this.ns =
            namespaceName == null ? undefined : namespaceName.split("/").filter(s => s.length > 0);
    }
    ...
    public outputPropertyName(
        _schema: NormalizedSchema,
        propertyName: string,
        nullable: boolean
    ): void {
        this.processor.outputKey(propertyName, nullable).output(": ");
    }
    ...
    public outputArrayedType<T>(
        schema: NormalizedSchema,
        types: T[],
        output: (t: T, index: number) => void,
        terminate: boolean,
        outputOptional = true
    ): void {
        if (!terminate) {
    ...
    public outputPrimitiveTypeName(
        schema: NormalizedSchema,
        typeName: string,
        terminate = true,
        outputOptional = true
