    ...
    function loadChannel(channel) {
        const handler = e => {
            if (
                channel !== publicPath &&
                confirm(
                    "The webpanel has failed to load from " +
                        channel +
                        ", this may be due to a misconfiguration. Press OK to load the webpanel from the fallback. Error: \n" +
                        e
                )
            ) {
        ...
        try {
            ...
            fetch(channel + "webpanelmanifest.json")
                .then(e => {
                    e.json()
                        .then(data => {
        ...
        fetch("channel.json", {
            ...
            e.json()
                ...
                .catch(e => {
                    if (
                        confirm(
                            "An error has occured within the webpanel bootstraper. Press OK to load the webpanel from the fallback. Error:\n" +
                                e
                        )
                    ) {
                        loadChannel(publicPath);
